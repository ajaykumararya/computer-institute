...
class Post_ajax extends Student_Controller {
...
// 		echo '<script>alert("Result successfully create.");location.href="'.site_url('admin/create_result').'"</script>';
			...
			if($check_auth_client == true){
			    ...
			    if($response['status'] == 200){
	               ...
	               $data = array(
					   ...
					   'center_course_show'=>json_encode([])
	...
	function center_registration_old()
	{
		$method = $_SERVER['REQUEST_METHOD'];
		if($method != 'POST'){
			json_output(400,array('status' => 400,'message' => 'Bad request.'));
    ...
    function test_enrl(){
        ...
        //             // $centerid = $prefix.time();
	       ...
	       $profile =  get_all_profile();
                    ...
                    echo $centerid =  preg_replace('/\s+/', '', $centeridx);
    }
    function get_center_courses(){
        echo json_encode(['option' => $this->center_model->get_center_course_options($this->input->post_get('center_id'))]);
    }
  function update_student_registration(){
       echo json_encode($this->student_data($_SESSION['loginid']));
  }
    function student_registration(){
        echo json_encode($this->student_data($_SESSION['loginid']));
    }
    public function student_registration_backup()
    {
        // die('here');
        $method = $_SERVER['REQUEST_METHOD'];
        if ($method != 'POST') {
...
/*------------- start student update --------------------------------*/
    public function update_student_registration_backup()
    {
        // die('here');
        $method = $_SERVER['REQUEST_METHOD'];
        if ($method != 'POST') {
	...
	function add_courses()
	{
		...
		if($method != 'POST'){
    		...
    		/*------------ start add courses ---------------------------------------*/
                                ...
                                if($_POST['yearly'] == 1 OR $_POST['yearly'] == 3){
                    		        $year=0;
                    		        $month=$_POST['duration'];
                    		    }else{
                    		        $year = $_POST['duration'];
                    		    ...
                    		    $ttlfee = $_POST['fees'];
                    		    if($_POST['yearly'] == 2 OR $_POST['yearly'] == 3){
                    		        $ttlfee = $ttlfee*$_POST['duration'];
                    		    }
